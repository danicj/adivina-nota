...
- Dibujo: `centerX = canvas.width/2`, `middleLineY = canvas.height/2` y `lineSpacing` se ajustan al tamaño del canvas, pero `staffInicio=40` y `staffFin=320` están HARDCODEADOS. Si cambias `width` del canvas en `index.html`, revisa esas constantes (también `radioNota`/`extensionLinea`). `dibujarPentagrama()` pinta el glifo de clave (𝄞/𝄢) vía `dibujarGlifo(clave)`, que dibuja el trazo vectorial embebido en `GLIFOS` (extraído de Noto Music, OFL) con `Path2D`: render idéntico en todos los dispositivos sin depender de fuentes del sistema. Tamaño/posición por clave se ajusta en el objeto `glifo` (justo debajo de `staffFin`); `tam` es la altura final en px.
