...
- Dibujo: `centerX = canvas.width/2`, `middleLineY = canvas.height/2` y `lineSpacing` se ajustan al tamaño del canvas, pero `staffInicio=40` y `staffFin=320` están HARDCODEADOS. Si cambias `width` del canvas en `index.html`, revisa esas constantes (también `radioNota`/`extensionLinea`). `dibujarPentagrama()` pinta el glifo de clave (𝄞/𝄢) vía `dibujarGlifo(clave)`, que mide el glifo con `measureText` y lo escala para que su altura final (`tam` en el objeto `glifo`, justo debajo de `staffFin`) sea idéntica en todos los dispositivos.
